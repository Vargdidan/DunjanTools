...
func _ready():
	Global.connect("ChangedMap", self, "_recieved_change_map")
...
func _recieved_change_map():
	if (get_tree().get_network_peer() != null):
		if ClientVariables.DMRole:
			save_battlemap()
			rpc("change_map", ClientVariables.SelectedMap)
	else:
		save_battlemap()
		change_map(ClientVariables.SelectedMap)

remotesync func change_map(_map):
	ClientVariables.ResetVariables()
	ClientVariables.SelectedMap = _map
	token_counter = 0
...
func save_battlemap():
	...
	battlemap.open(ClientVariables.DataPath + map.current_map.split(".")[0] + ".dat", File.WRITE)
	var json_tokens = []
	for token in tokens.get_children():
